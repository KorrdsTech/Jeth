...
module.exports = async function onMessage(message) {
  const guildDocument = await this.database.Guilds.findById(message.guild.id)
  ...
  if (message.author.bot) {
    ...
    const Users = await this.database.Users.findById(member?.id)
    if (!Users) return
  ...
  const Users = await this.database.Users.findById(message.author.id)
