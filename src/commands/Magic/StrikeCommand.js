    ...
    switch (args[0]) {
      case 'add': {
        ...
        if (guildDocument.strike >= 3) {
          ...
          guildDocument.save().then(() => message.reply(`O usuário \`${usuario.tag}\` foi adicionado da blacklist.`))
        }
        message.reply(`O usuário \`${usuario.tag}\` acabou levando strike (${guildDocument.strike}))`)
      ...
      case 'remove': {
        const usuario = await this.client.users.fetch(args[1].replace(/[<@!>]/g, ''))
        const guildDocument = await this.client.database.user.getOrCreate(usuario.id)
        message.reply(`${usuario.tag} Teve seu strike removido! <:a_blurplecertifiedmoderator:856174396225355776>`)
        guildDocument.strike -= 1
        guildDocument.save()
        if (guildDocument.strike < 3) {
          guildDocument.blacklist = false
          guildDocument.blacklistReason = null
          guildDocument.save().then(() => message.reply(`O usuário \`${usuario.tag}\` foi removido da blacklist.`))
        }
        message.reply(`O usuário \`${usuario.tag}\` teve 1 strike removido (${guildDocument.strike})`)
      }
        break
      default: {
