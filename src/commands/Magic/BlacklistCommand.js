...
module.exports = class blacklist extends Command {
  ...
  async run(message, args) {
    ...
    //   .setDescription(`:do_not_litter: **Você foi blacklisted ${message.guild.name} :no_entry_sign:** \nO que isto significa ? você não poderá mais fazer parte dos servidores que apoiam a network da Moderadora, por quebrar um dos termos de serviço do discord, este tipo de punição não oferece appeal e se você se encontra nesta situação provavelmente terá sua conta encerrada.`)
    //   .setColor('BLACK')
    //   .addFields('<:pepe:651487933148299291> **Staffer:**', `${message.author}`)
    //   .addFields('📝 Motivo:', `${reason}`)
    //   .setFooter({ text: 'https://discord.com/guidelines・Discord da Moderadora 🛠' })
