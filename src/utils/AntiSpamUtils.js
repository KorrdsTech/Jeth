...
module.exports = class AntiSpamUtils {
  ...
  static checkPossibly(message) {
    const botrole = (message.guild.me.roles.highest.rawPosition < message.member.roles.highest.rawPosition) || (!message.member.permissions.has('MANAGE_GUILD') && message.guild.ownerId !== message.author.id)
    if (botrole) {
      return true
    } else {
      return false
